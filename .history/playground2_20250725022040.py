...
class AVNNPokemonModel(torch.nn.Module):
    def __init__(self, input_dim, latent_dim=64):
        ...
        self.model = Sequential(
            ...
            AVNNType2Linear(latent_dim, latent_dim),
            AVNNLinearBlock(latent_dim),
            AVNNLinearToConv2dBridge(channels=4, height=4, width=4),  # 4x4x4 = 64
            AVNNConv2dBlock(in_channels=4, out_channels=4),
            AVNNConv2dToLinarBridge(dim=latent_dim, channels=4, height=4, width=4),
            AVNNLinearBlock(latent_dim),
        )
