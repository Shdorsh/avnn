...
class TagValueTokenizer:
    ...
    def save_vocab(self):
        filetext = dump(self.vocab)
        with open("vocab.json") as fd:
            write(fd, filetext)
    ...
    def load_vocab(self, file):
        if not isfile(file):
            FileNotFoundError(f"The given file {file} could not be found!")
            return
        with open(file) as fd:
            try:
                self.vocab = load(fd.read())
            except:
                JSONDecodeError("Error: Could not parse file {file} to JSON!")
    ...
    def tokenize(self, data_string):
        ...
        for row in data_string.strip().split(self.row_delimiter):
            ...
            for entry in row.split(self.column_separator):
                ...
                tag = self.vocab[tag_value[0].strip()]
                val = self.vocab[tag_value[1].strip()] if len(tag_value) > 1 else 0
