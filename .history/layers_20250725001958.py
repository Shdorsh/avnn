...
class AVNNType1Linear(Module):
    ...
    def forward(self, input_tensor):
        if input_tensor.ndim > 3:
            print("Input tensor dimensions: ", input_tensor.ndim)
        ...
        for i in range(batch_size):
            val = self.derive_mode(activation_in[i].tolist(), carry_in[i].tolist())
...
class AVNNType1Conv2d(nn.Module):
    """Type 1: Value (first float) is used as 'activator', Meaning (second float) is used as carry,
    meaning literally just being carried into the next tensor"""
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, derive_mode=derived_adjustedmean, activation=relu):
        super().__init__()
        # X-path conv
    ...
    def forward(self, input_tensor):
        ...
        for b in range(B):
            for c in range(C):
                for i in range(Hp):
                    for j in range(Wp):
                        x_list = patches_x[b, c, i, j].tolist()
                        y_list = patches_y[b, c, i, j].tolist()
